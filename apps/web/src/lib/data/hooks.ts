...
} from "@/lib/dc-data/prep-ready-overrides";
...
export function useCalls() {
  const prepReadyByCallId = useCallWorkflowStore((s) => s.prepReadyByCallId);
  ...
  const data = useMemo(
    () => (query.data ? applyPrepReadyToCalls(query.data, prepReadyByCallId) : query.data),
    [query.data, prepReadyByCallId]
  );
...
export function useCall(callId: string) {
  ...
  const data = useMemo(
    () => (query.data ? applyPrepReadyToCall(query.data, prepReadyByCallId) : query.data),
    [query.data, prepReadyByCallId]
  );
