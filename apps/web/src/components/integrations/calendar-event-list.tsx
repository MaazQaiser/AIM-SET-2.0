        ...
        <div className="flex items-start justify-between gap-2">
          ...
          {call.meetingUrl && (
            <a
              ...
              className="shrink-0"
            >
              <Button size="sm" className="h-6 text-xs gap-1 bg-success hover:bg-success/90 text-white">
        ...
        {externalAttendees.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            ...
            {externalAttendees.length > 3 && (
              <span className="text-[10px] text-muted-foreground">+{externalAttendees.length - 3} more</span>
            )}
          </div>
        )}
      </div>
...
export function CalendarEventList({ className }: CalendarEventListProps) {
  ...
  if (error) {
    return (
      <div className={cn("rounded-md border border-destructive/30 bg-destructive/5 px-4 py-3 text-sm text-destructive", className)}>
  ...
  return (
    <div className={cn("divide-y rounded-md border overflow-hidden", className)}>
      ...
      {data?.syncedAt && (
        <div className="px-4 py-2 text-[10px] text-muted-foreground bg-muted/20">
