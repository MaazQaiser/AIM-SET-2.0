...
def _apply_signals(
    ...
    *,
    ...
    for item_id, keywords, tier_status in SIGNAL_RULES:
        ...
        # self-authority phrase is detected.
        if item_id == "authority":
            matched = bool(_AUTHORITY_RE.search(text) or _SELF_AUTHORITY_RE.search(text))
        else:
    ...
    for item_id, tier_status in matched_statuses.items():
        for it in items:
            ...
            # Additionally, authority can only reach "confirmed" if the customer
            # explicitly claims decision authority (self-authority pattern).
            if item_id == "authority":
                if not value:
                    continue
                if new_status == "confirmed" and not _SELF_AUTHORITY_RE.search(text):
                    new_status = "partial"
            ...
            # Budget cannot reach "confirmed" without actual currency evidence.
            if item_id == "budget":
                if not value:
                    if new_status != it.status:
                        # Cap at "partial" without currency evidence
                        capped = "partial" if _status_rank(new_status) > _status_rank("partial") else new_status
                        if _status_rank(capped) > _status_rank(it.status):
                            it.status = capped
                            if item_id not in changed:
                                changed.append(item_id)
                    continue
                # Even with value, require currency pattern for "confirmed"
                if new_status == "confirmed" and not money_hit:
                    new_status = "partial"
